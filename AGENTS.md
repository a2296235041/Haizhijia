...
本文件面向修改模板的 Agent 和工程维护者。目标是保持模板轻量、配置驱动、离线可用，并保护 Gamefy SDK、AI 请求和存储 schema 的稳定边界。命令均在模板仓库根目录执行；实例化游戏的根目录是 `/workspace`。在 Game Studio 源码仓库维护本模板时，先进入 `templates/blank/`。

## 阅读顺序

先读本文件，再按任务读取最少上下文，不要求每次通读全部模块。
...
```sh
for file in publish/*.js; do node --check "$file"; done
git diff --check -- .
```

不要添加 test/spec 文件，不要修改当前模板仓库之外的文件，不要提交 commit。内容或 UI 改动手动覆盖受影响流程；平台、存储或 AI 契约变化时再覆盖玩家资料成功与降级、AI 成功与无效输出、重复点击、KV 与本地回退、损坏存档和启动错误。发布前仍执行完整流程。
