...
模板当前不需要 `assets`。如果作品加入图片、字体、音频或 JSON，请放入 `publish/assets/`，使用 ASCII 文件名和相对路径，不要引用 CDN 或远程字体。请合理控制并压缩素材体积，尤其关注首屏资源和单个文件，避免弱网加载过慢或分发困难；单个栅格图片不得超过 1.5 MiB。

加入音频时按 `templates/AUTHORING.md` 第 6 节执行：文件播放走 `dzmm.audio`（禁止 `new Audio()`），素材用 MP3 放在 `publish/assets/audio/` 并在 `publish/assets/audio/manifest.json` 中登记 key；素材获取与生成见 Coding Agent 的 `game-audio-gen` skill。
