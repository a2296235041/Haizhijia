 ...
 * タイル＆全キャラ＆追加背景：TileAndCharsAndParallaxPlus
 * タイル　　　　　　　　　：Tile
 * タイル＆背景　　　　　　：TileAndParallax
 * 背景　　　　　　　　　　：Parallax
 * 追加背景（ParallaxPlus）：ParallaxPlus
 * 特定追加背景（id）　　　：SpecificParallaxPlus
 * 特定追加背景以外（id）　：ExcludeSpecificParallaxPlus
 * 全キャラ　　　　　　　　：Chars
 ...
 * @option タイル＆全キャラ＆追加背景
 * @value TileAndCharsAndParallaxPlus
 * @option タイル
 * @value Tile
 * @option タイル＆背景
 ...
 * @option 背景
 * @value Parallax
 * @option 追加背景（ParallaxPlus）
 * @value ParallaxPlus
 * @option 特定追加背景(id)
 * @value SpecificParallaxPlus
 * @option 特定追加背景以外(id)
 * @value ExcludeSpecificParallaxPlus
 * @option 全キャラ
 * @value Chars
 * @option 特定キャラ(id)
 * @value SepcificChar
 ...
 * @arg waitFrames
 * @text ウェイト時間(フレーム)
 * @desc フィルター作成後にウェイトする時間（フレーム数）。0の場合はウェイトしない
 * @type number
 * @min 0
 * @max 999
 * @default 0
 ...
 * @arg waitFrames
 * @text ウェイト時間(フレーム)
 * @desc フィルターパラメータ設定後にウェイトする時間（フレーム数）。0の場合はウェイトしない
 * @type number
 * @min 0
 * @max 999
 * @default 0
 ...
 * @desc -----
 * 
 * @command zappingNoise
 * @text ザッピングノイズ
 * @desc カメラザッピング用ノイズ。マップとキャラクターを完全に隠すノイズを指定フレーム数表示します（ピクチャは見える）
 * 
 * @arg duration
 * @text 表示フレーム数
 * @desc ノイズを表示するフレーム数（60=約1秒）
 * @type number
 * @min 1
 * @default 6
 * 
 * @arg noiseIntensity
 * @text ノイズ強度
 * @desc ノイズの強さ。1.0でほぼ完全に隠れます
 * @type string
 * @default 1.5
 * 
 * @arg includeParallax
 * @text 背景(Parallax)も隠す
 * @desc ONにすると遠景/背景もノイズで隠れます
 * @type boolean
 * @default true
 * 
 * @arg transferEnabled
 * @text 場所移動を行う
 * @desc ONにすると場所移動を同時に行います（フェードなし）
 * @type boolean
 * @default false
 * 
 * @arg transferMapId
 * @text 移動先マップID
 * @desc 移動先のマップID（0で現在のマップ、変数使用可: v1など）
 * @type string
 * @default 0
 * 
 * @arg transferX
 * @text 移動先X座標
 * @desc 移動先のX座標（変数使用可: v1など）
 * @type string
 * @default 0
 * 
 * @arg transferY
 * @text 移動先Y座標
 * @desc 移動先のY座標（変数使用可: v1など）
 * @type string
 * @default 0
 * 
 ...
 * TileAndCharsAndParallaxPlus
 * Tile
 * TileAndParallax
 * Parallax
 * ParallaxPlus
 * SpecificParallaxPlus
 * ExcludeSpecificParallaxPlus
 * Chars
 * SepcificChar
 * ExcludeSepcificChar
 * Pictures
 ...
 * @value TileAndCharsAndParallax
 * @option TileAndCharsAndParallaxPlus
 * @value TileAndCharsAndParallaxPlus
 * @option Tile
 * @value Tile
 ...
 * @option ParallaxPlus
 * @value ParallaxPlus
 * @option SpecificParallaxPlus
 * @value SpecificParallaxPlus
 * @option ExcludeSpecificParallaxPlus
 * @value ExcludeSpecificParallaxPlus
 * @option Chars
 * @value Chars
 * @option SepcificChar
...
(function() {
	...
	PluginManager.registerCommand(pluginName, "createFilter", function(args) {
		...
		const filterTargetIds = parseTargetIdArray(args.targetIds, this, filterTarget);
		const posRefTargetId = parseFilterPosRefId(args.positionReferenceTargetId, this);
		const waitFrames = parseInt(args.waitFrames || 0);
		console.log("FilterController: Plugin command called with:", {filterId, filterType, filterTarget, filterTargetIds, waitFrames});
	...
	const parseTargetIdArray = function(idTextArrayText, interpreter, filterTarget) {
		if (idTextArrayText == "" || idTextArrayText == null) return null;

		const idTextArray = JSON.parse(idTextArrayText), targetIdArray = [];
		console.log("FilterController: Parsing target IDs:", idTextArray, "for target type:", filterTarget);
		
		for (let i = 0; i < idTextArray.length; i++) {
			// ParallaxPlusの場合は文字列IDをそのまま使用
			if (filterTarget === "SpecificParallaxPlus" || filterTarget === "ExcludeSpecificParallaxPlus") {
				targetIdArray.push(String(idTextArray[i]));
			} else {
				// 既存の数値処理
		...
		console.log("FilterController: Parsed target IDs:", targetIdArray);
	...
	PluginManager.registerCommand(pluginName, "setFilter", function(args) {
		const filterId = String(args.filterId);
	...
	// ザッピングノイズコマンド - マップとキャラを隠す一時的ノイズ（ピクチャは見える）
	// 場所移動機能付き
	PluginManager.registerCommand(pluginName, "zappingNoise", function(args) {
		const duration = parseInt(args.duration) || 6;
		const noiseIntensity = parseFloat(args.noiseIntensity) || 1.5;
		const includeParallax = parseBoolean(args.includeParallax);
		const transferEnabled = parseBoolean(args.transferEnabled);
		
		// 場所移動が有効な場合
		if (transferEnabled) {
			let mapId = parseNumberOrDefault(args.transferMapId, true);
			if (mapId === null || mapId === 0) mapId = $gameMap.mapId(); // 0またはnullなら現在のマップ
			const x = parseNumberOrDefault(args.transferX, true) || 0;
			const y = parseNumberOrDefault(args.transferY, true) || 0;
			const direction = parseInt(args.transferDirection) || 0;
			
			// 同じマップ内の移動の場合は直接座標変更（場所移動コマンドを使わない）
			if (mapId === $gameMap.mapId()) {
				// $gameScreenでザッピングノイズを開始
				$gameScreen.startZappingNoise(duration, noiseIntensity, includeParallax);
				...
				// 異なるマップへの移動の場合
				// $gameScreenでザッピングノイズを開始（マップ遷移後も継続）
				$gameScreen.startZappingNoiseWithTransfer(duration, noiseIntensity, includeParallax, mapId, x, y, direction);
			}
		} else {
			// 場所移動なしの場合
			$gameScreen.startZappingNoise(duration, noiseIntensity, includeParallax);
	...
	Filter_Controller.targetType["TileAndCharsAndParallaxPlus"] = "TileAndCharsAndParallaxPlus";
	...
	Filter_Controller.targetType["ParallaxPlus"] = "ParallaxPlus";
	Filter_Controller.targetType["SpecificParallaxPlus"] = "SpecificParallaxPlus";
	Filter_Controller.targetType["ExcludeSpecificParallaxPlus"] = "ExcludeSpecificParallaxPlus";
	...
	_defaultParam["outline"]        = [2,255,255,255];
	...
	_updateFilterHandler["outline"] = function(filter, cp) {
		filter.thickness = cp[0];
		// r,g,b to number(0x); ~~ is faster than floor()
		filter.color = (~~cp[1])*65536 + (~~cp[2])*256 + (~~cp[3]);
	};
	...
	Game_Map.prototype.createFilter = function(id, filter, targetType, targetIds, posRefTargetId) {
		console.log("FilterController: Creating filter with:", {id, filter, targetType, targetIds});
		...
		console.log("FilterController: Filter created successfully, total filters:", this._filterConArr.length);
	};
	...
	//=======================
	// Game_Screen
	// ザッピングノイズは場所移動をまたいで維持されるよう$gameScreenで管理
	//=======================
	...
	Game_Screen.prototype.update = function(sceneActive) {
		...
		this.updateZappingNoise();
	};

	// ザッピングノイズ開始
	Game_Screen.prototype.startZappingNoise = function(duration, intensity, includeParallax) {
		const filterId = "_ZAPPING_NOISE_";
		const targetType = includeParallax ? "TileAndCharsAndParallax" : "TileAndChars";
		
		// 既存のザッピングノイズがあれば削除
		$gameMap.eraseFilter(filterId);
		
		// ノイズフィルター作成
		$gameMap.createFilter(filterId, "noise", targetType, null, null);
		$gameMap.setFilter(filterId, [intensity]);
		
		// フィルターコントローラの_timeを設定してseedを毎フレーム更新させる
		const fc = $gameMap.getFilterController(filterId);
		if (fc) {
			fc._time = 2; // seedが更新される閾値(1)を超える値に設定
		}
		
		// タイマー設定（$gameScreenで管理）
		this._zappingNoiseData = {
			filterId: filterId,
			remainingFrames: duration,
			intensity: intensity,
			includeParallax: includeParallax,
			pendingTransfer: false
		};
	};

	// ザッピングノイズ開始（マップ移動付き）
	Game_Screen.prototype.startZappingNoiseWithTransfer = function(duration, intensity, includeParallax, mapId, x, y, direction) {
		const filterId = "_ZAPPING_NOISE_";
		const targetType = includeParallax ? "TileAndCharsAndParallax" : "TileAndChars";
	...
	// ザッピングノイズ更新
	Game_Screen.prototype.updateZappingNoise = function() {
		if (!this._zappingNoiseData) return;
		
		const data = this._zappingNoiseData;
		
		// フィルターコントローラを取得
		let fc = $gameMap.getFilterController(data.filterId);
		
		// フィルターが存在しない場合は再作成（場所移動後対応）
		if (!fc) {
			this.forceApplyZappingNoise();
			fc = $gameMap.getFilterController(data.filterId);
		}
		
		// ノイズのseedを毎フレーム強制更新（停止防止）
		if (fc) {
			fc._time = 2; // 常にseedが更新される状態を維持
		}
		
		// シーン上のフィルターオブジェクトのseedも直接更新（マップ遷移時対策）
		const scene = SceneManager._scene;
		if (scene && scene._TKMFilters) {
			const filter = scene._TKMFilters.find(f => f._controller && f._controller._id === data.filterId);
			if (filter && filter.seed !== undefined) {
				filter.seed = Math.random() * 3;
			}
		}
		...
		if (data.remainingFrames <= 0) {
			$gameMap.eraseFilter(data.filterId);
			this._zappingNoiseData = null;
		}
	};

	// ザッピングノイズ強制適用（場所移動時のラグ防止）
	Game_Screen.prototype.forceApplyZappingNoise = function() {
		if (!this._zappingNoiseData) return;
		
		const data = this._zappingNoiseData;
		const targetType = data.includeParallax ? "TileAndCharsAndParallax" : "TileAndChars";
		
		// 既存のフィルターがあれば削除
		$gameMap.eraseFilter(data.filterId);
		
		// フィルター作成
		$gameMap.createFilter(data.filterId, "noise", targetType, null, null);
		$gameMap.setFilter(data.filterId, [data.intensity]);
		
		// フィルターコントローラの_timeを大きな値に設定してseedを即座に更新させる
		const fc = $gameMap.getFilterController(data.filterId);
		if (fc) {
			fc._time = 2; // seedが更新される閾値(1)を超える値に設定
		}
	};
	...
	Game_Map.prototype.setup = function(mapId) {
		...
		// 場所移動時、ザッピングノイズが有効なら即座に再適用
		if ($gameScreen && $gameScreen._zappingNoiseData) {
			$gameScreen.forceApplyZappingNoise();
		}
	};
	...
	// defenition of each TargetType's target
	{
		...
		targetGetter[Type.TileAndCharsAndParallaxPlus] = function(targetIds) {
			const targets = [];
			if (this._spriteset && this._spriteset._baseSprite) {
				targets.push(this._spriteset._baseSprite);
			}
			// NRP_ParallaxesPlusで作成された追加背景も含める
			if (this._spriteset && this._spriteset._parallaxPlus) {
				this._spriteset._parallaxPlus.forEach(parallax => targets.push(parallax));
			}
			return targets;
		};
		...
		targetGetter[Type.ParallaxPlus] = function(targetIds) {
			const targets = [];
			// NRP_ParallaxesPlusで作成された追加背景を対象にする
			if (this._spriteset && this._spriteset._parallaxPlus) {
				this._spriteset._parallaxPlus.forEach(parallax => targets.push(parallax));
			}
			return targets;
		...
		targetGetter[Type.SpecificParallaxPlus] = function(targetIds) {
			const targets = [];
			// NRP_ParallaxesPlusで作成された特定IDの追加背景を対象にする
			if (!this._spriteset || !this._spriteset._parallaxPlus) {
				console.log("FilterController: No _parallaxPlus found");
				return targets;
			}
			
			const parallaxPlus = this._spriteset._parallaxPlus;
			console.log("FilterController: Found parallaxPlus array:", parallaxPlus.length);
			console.log("FilterController: Searching for targetIds:", targetIds);
			
			// 各parallaxのIDをログ出力してデバッグ
			parallaxPlus.forEach((p, index) => {
				console.log(`FilterController: Parallax[${index}] id: "${p.id}"`);
			});
			
			targetIds.forEach(targetId => {
				console.log(`FilterController: Looking for targetId: "${targetId}"`);
				const parallax = parallaxPlus.find(p => p.id === targetId);
				if (parallax) {
					console.log(`FilterController: Found matching parallax for id: "${targetId}"`);
					targets.push(parallax);
				} else {
					console.log(`FilterController: No matching parallax found for id: "${targetId}"`);
				}
			});
			
			console.log("FilterController: Final targets count:", targets.length);
			return targets;
		};

		targetGetter[Type.ExcludeSpecificParallaxPlus] = function(targetIds) {
			if (!this._spriteset || !this._spriteset._parallaxPlus) return [];
			
			const parallaxPlus = this._spriteset._parallaxPlus;
			const targets = parallaxPlus.slice(); // 配列をコピー
			targetIds.forEach(targetId => {
				const parallax = parallaxPlus.find(p => p.id === targetId);
				if (parallax) {
					const index = targets.indexOf(parallax);
					if (index >= 0) {
	...
	// 場所移動後、シーン開始時にザッピングノイズを即座に再適用
	const _Scene_Map_start = Scene_Map.prototype.start;
	Scene_Map.prototype.start = function() {
		_Scene_Map_start.apply(this, arguments);
		// ザッピングノイズが有効な場合、即座にフィルターを再作成・適用
		if ($gameScreen._zappingNoiseData) {
			$gameScreen.forceApplyZappingNoise();
			this.updateTKMfilters(); // 即座にフィルターを適用
		}
	};
...
_FNMap["outline"]        = PIXI.filters.OutlineFilter;
